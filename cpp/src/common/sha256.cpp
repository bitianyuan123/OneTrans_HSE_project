...
std::vector<uint8_t> sha256(const void* data, size_t len) {
    ...
    // padding：0x80 + 0..63 个 0 + 8 字节大端 bit 长度
    ...
    for (int i = 0; i < 8; ++i) buf[padded - 1 - i] = static_cast<uint8_t>((bits >> (8 * i)) & 0xFF);
...
std::string sha256_hex(const std::string& data) { return sha256_hex(data.data(), data.size()); }

uint64_t stable_hash64(const std::string& key) {
    auto digest = sha256(key);
    uint64_t v = 0;
    for (int i = 0; i < 8; ++i) v = (v << 8) | digest[static_cast<size_t>(i)];
    return v;
}
